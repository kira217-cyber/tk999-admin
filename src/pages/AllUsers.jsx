...
export default function AllUser() {
  ...
  const fetchMasterAffiliates = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch master affiliates");
      const data = await res.json();
      setMasterAffiliates(data.users || []);
    } catch (err) {
      console.error("Failed to load Master Affiliates for dropdown");
    }
  };
  ...
  const handleToggleClick = async (user) => {
    ...
    if (!window.confirm(isActive ? "Deactivate user?" : "Activate user?")) return;

    try {
      const res = await fetch(`${API_URL}/api/toggle-user/${user._id}`, {
        method: "PATCH",
  ...
  const handlePasswordSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`${API_URL}/api/update-user-credentials/${viewUser._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleCommissionSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`${API_URL}/api/update-user-commission/${selectedUser._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(commissionForm),
      });
      if (!res.ok) throw new Error("Commission update failed");
  ...
  // Animation variants (same as SuperAffiliate)
  const cardVariants = {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-emerald-950/20 to-black p-4 sm:p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-emerald-300/80 text-lg sm:text-xl">
            Manage regular users with full commission control
          </p>
        </div>
              ...
              <table className="w-full min-w-[1100px] text-left">
                <thead>
                  <tr className="bg-gradient-to-r from-emerald-950/80 to-gray-900/80 border-b border-emerald-800/50">
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Username</th>
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Email</th>
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">WhatsApp</th>
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Balance</th>
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Status</th>
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Toggle</th>
                    <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Edit</th>
                    {/* <th className="px-6 py-5 text-emerald-300 font-semibold cursor-default">Commission</th> */}
                  </tr>
                </thead>
                <tbody>
                  {users.map((user, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-5 text-gray-300 break-all">{user.email}</td>
                        <td className="px-6 py-5 text-gray-300">{user.whatsapp}</td>
                        ...
                        {/* Uncomment if you want commission back */}
                        {/* <td className="px-6 py-5 text-center">
                          <button
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      {/* Uncomment if you want commission button back */}
                      {/* <button
                        onClick={() => handleCommissionClick(user)}
      ...
      {/* Create User Modal - Scrollable on Mobile */}
      {createModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          ...
          >
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-5">
              <div>
                <label className="block text-emerald-300 font-medium mb-2">Username *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-emerald-300 font-medium mb-2">Email *</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-emerald-300 font-medium mb-2">Password *</label>
              ...
              <div>
                <label className="block text-emerald-300 font-medium mb-2">WhatsApp *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-emerald-300 font-medium mb-2">
                  Referred By (Master Affiliate) *
                </label>
                <select
                  value={createForm.referredBy}
                  onChange={(e) => setCreateForm({ ...createForm, referredBy: e.target.value })}
                  required
              ...
              {/* Commission fields commented out - same as original */}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-5">
              ...
              <div>
                <label className="block text-emerald-300 font-medium mb-2">New Password</label>
                <div className="relative">
                  <input
      ...
      {/* Commission Modal (kept commented as in your original) */}
